...
class StringBuilderTest {
    ...
    fun appendCodePoint() {
        StringBuilder().let { sb ->
            ...
            sb.appendCodePoint(0xD777)
            sb.appendCodePoint(0xD800)
            sb.appendCodePoint(0xE000)
            sb.appendCodePoint(0x10000)
            sb.appendCodePoint(0x10FFFF)
            ...
            assertFailsWith<IllegalArgumentException> { sb.appendCodePoint(-1) }
            assertFailsWith<IllegalArgumentException> { sb.appendCodePoint(0x110000) }
        }
    }

    @Test
    fun codePointsBefore() {
        val sb = StringBuilder().apply {
            appendCodePoint(0xDC00)     // low surrogate
            appendCodePoint(0xD800)     // high surrogate
            appendCodePoint(0xDC00)     // low surrogate
            appendCodePoint(0xFFFF)
            appendCodePoint(0xD801)     // high surrogate
            appendCodePoint(0xFFFF)
            appendCodePoint(0xDC01)     // low surrogate
        }

        assertEquals(0xDC01, sb.codePointBefore(sb.length /*7*/))
        assertEquals(0xFFFF, sb.codePointBefore(6))
        assertEquals(0xD801, sb.codePointBefore(5))
        assertEquals(0x10000, sb.codePointBefore(3))
        assertEquals(0xD800, sb.codePointBefore(2))
        assertEquals(0xDC00, sb.codePointBefore(1))

        assertFailsWith<IndexOutOfBoundsException> { sb.codePointBefore(0) }
        assertFailsWith<IndexOutOfBoundsException> { sb.codePointBefore(-1) }
        assertFailsWith<IndexOutOfBoundsException> { sb.codePointBefore(sb.length + 1) }
    }
    ...
    fun codePointsCount() {
        ...
        assertEquals(6, sb.codePointCount(0, sb.length /*7*/))
        assertEquals(5, sb.codePointCount(1, sb.length))
        assertEquals(2, sb.codePointCount(0, 2))
        assertEquals(2, sb.codePointCount(0, 3))
        assertEquals(1, sb.codePointCount(1, 3))
        assertEquals(4, sb.codePointCount(3, sb.length))

        assertFails { sb.codePointCount(0, -1) }
        assertFails { sb.codePointCount(-1, 1) }
        assertFails { sb.codePointCount(2, 1) }
        assertFails { sb.codePointCount(0, sb.length + 1) }
    }
    ...
    fun offsetByCodePoints() {
        ...
        assertEquals(sb.length /*7*/, sb.offsetByCodePoints(0, 6))
        assertEquals(sb.length, sb.offsetByCodePoints(sb.length, 0))
        assertEquals(sb.length, sb.offsetByCodePoints(1, 5))
        assertEquals(sb.length, sb.offsetByCodePoints(2, 5))
        assertEquals(3, sb.offsetByCodePoints(0, 2))
        assertEquals(4, sb.offsetByCodePoints(1, 2))
        assertEquals(6, sb.offsetByCodePoints(3, 3))

        assertEquals(0, sb.offsetByCodePoints(sb.length /*7*/, -6))
        assertEquals(1, sb.offsetByCodePoints(sb.length, -5))
        assertEquals(0, sb.offsetByCodePoints(2, -2))
        assertEquals(0, sb.offsetByCodePoints(3, -2))
        assertEquals(4, sb.offsetByCodePoints(sb.length, -3))

        assertFailsWith<IndexOutOfBoundsException> { sb.offsetByCodePoints(-1, 1) }
        assertFailsWith<IndexOutOfBoundsException> { sb.offsetByCodePoints(sb.length + 1, -1) }

        assertFails { sb.offsetByCodePoints(0, -1) }
        assertFails { sb.offsetByCodePoints(sb.length, -7) }
        assertFails { sb.offsetByCodePoints(0, 7) }
    }
    ...
    fun codePointAtIndex() {
        val sb = StringBuilder().apply {
            ...
            appendCodePoint(0xD801)     // high surrogate
            appendCodePoint(0xFFFF)
            appendCodePoint(0xD802)     // low surrogate
        }

        assertEquals(0xDC00, sb.codePoint(0))
        assertEquals(0x10000, sb.codePoint(1))
        assertEquals(0xDC00, sb.codePoint(2))
        assertEquals(0xD801, sb.codePoint(4))
        assertEquals(0xFFFF, sb.codePoint(5))
        assertEquals(0xD802, sb.codePoint(6))

        assertFailsWith<IndexOutOfBoundsException> { sb.codePoint(-1) }
        assertFailsWith<IndexOutOfBoundsException> { sb.codePoint(sb.length) }
    }
