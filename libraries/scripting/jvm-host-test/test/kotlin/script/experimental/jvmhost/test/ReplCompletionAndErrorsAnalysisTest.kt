...
class ReplCompletionAndErrorsAnalysisTest : TestCase() {
    ...
    private class TestConf {
        ...
        class Run {
            ...
            class Expected(private val run: Run) {
                ...
                private val errors = mutableListOf<KotlinReplError>()
                ...
                fun addError(startLine: Int, startCol: Int, endLine: Int, endCol: Int, message: String, severity: String) {
                    ...
                    run.doErrorCheck
                    errors.push(KotlinReplError(startLine, startCol, endLine, endCol, message, severity))
                }
    ...
    data class ExpectedResult(
        val completions: List<CompletionVariant>, val completionsCompType: ComparisonType,
        val errors: List<KotlinReplError>, val errorsCompType: ComparisonType
    )
    ...
    private fun evaluateInRepl(
    ...
    ): List<ResultWithDiagnostics<Pair<List<CompletionVariant>, List<KotlinReplError>>>> {
        ...
        return snippets.map { (cursor, snippetText, doComplile, doComplete, doErrorCheck) ->
            ...
            val completionRes = if (doComplete && cursor != null) {
                compiler.complete(state, codeLine, cursor).filter { it.tail != "keyword" }
            } else {
                emptyList()
            }
