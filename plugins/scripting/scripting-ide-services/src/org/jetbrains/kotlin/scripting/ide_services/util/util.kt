...
internal inline fun <T> withMessageCollectorAndDisposable(
    ...
    val messageCollector =
        ScriptDiagnosticsMessageCollector(parentMessageCollector)
    return try {
        ...
        failure(
            messageCollector,
            ex.asDiagnostics(path = script?.locationId)
        )
    } finally {
...
internal inline fun <T> withMessageCollector(
    ...
    return try {
        ...
        failure(
            messageCollector,
            ex.asDiagnostics(path = snippet?.locationId)
        )
    }
}
...
internal fun getScriptKtFile(
    ...
    val scriptText = getMergedScriptText(snippet, scriptCompilationConfiguration)
    val virtualFile = ScriptLightVirtualFile(
        snippet.scriptFileName(snippet, scriptCompilationConfiguration),
        (snippet as? FileBasedScriptSource)?.file?.path, // TODO: should be absolute path here
        scriptText
    )
...
internal class SourceCodeImpl(val file: KtFile) : SourceCode {
    override val text: String
        get() = file.text
    override val name: String?
        get() = file.name
    override val locationId: String?
        get() = file.virtualFilePath
}
