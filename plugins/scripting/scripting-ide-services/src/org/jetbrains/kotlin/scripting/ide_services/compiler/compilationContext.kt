...
import org.jetbrains.kotlin.scripting.ide_services.util.IgnoredOptionsReportingState
import org.jetbrains.kotlin.scripting.ide_services.util.ScriptDiagnosticsMessageCollector
import org.jetbrains.kotlin.scripting.ide_services.util.reportArgumentsIgnoredFromRefinement
import org.jetbrains.kotlin.scripting.ide_services.util.reportArgumentsIgnoredGenerally
...
internal class ScriptingSamWithReceiverComponentContributor(private val annotations: List<String>) : StorageComponentContainerContributor {
...
internal fun SharedScriptCompilationContext.applyConfigure(): SharedScriptCompilationContext = apply {
    ...
    if (samWithReceiverAnnotations?.isEmpty() == false) {
        StorageComponentContainerContributor.registerExtension(
            environment.project,
            ScriptingSamWithReceiverComponentContributor(
                samWithReceiverAnnotations.map { it.typeName })
        )
    }
}
