...
class JvmIdeServicesTest : TestCase() {
    ...
    private fun checkContains(actual: Sequence<ReplCompletionVariant>, expected: Set<String>) {
    ...
    private fun checkDoesntContain(actual: Sequence<ReplCompletionVariant>, expected: Set<String>) {
        val variants = actual.map { it.displayText }.toHashSet()
        for (displayText in expected) {
            if (variants.contains(displayText)) {
...
private fun JvmTestRepl.compileAndEval(codeLine: SourceCode): Pair<ResultWithDiagnostics<ILinkedPushStack<out CompiledSnippet>>, EvaluatedSnippet?> {
    ...
    return compRes to evalRes?.valueOrNull()()
}
...
private fun assertEvalUnit(
    ...
    val valueResult = evalResult.valueOrNull()()
...
private fun checkCompile(repl: JvmTestRepl, line: String): ILinkedPushStack<KJvmCompiledScript<Any>>? {
