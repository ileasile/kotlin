...
class ReplCompletionAndErrorsAnalysisTest : TestCase() {
    ...
    private class TestConf {
        ...
        class Run {
            ...
            class Expected(private val run: Run) {
                private val variants = mutableListOf<ReplCompletionVariant>()
                ...
                fun addCompletion(text: String, displayText: String, tail: String, icon: String) {
                    ...
                    run.doComplete
                    variants.push(ReplCompletionVariant(text, displayText, tail, icon))
                }

                private val errors = mutableListOf<ReplDiagnosticMessage>()
                ...
                fun addError(startLine: Int, startCol: Int, endLine: Int, endCol: Int, message: String, severity: String) {
                    ...
                    run.doErrorCheck
                    errors.push(ReplDiagnosticMessage(startLine, startCol, endLine, endCol, message, severity))
                }
    ...
    data class ExpectedResult(
        val completions: List<ReplCompletionVariant>, val completionsCompType: ComparisonType,
        val errors: List<ReplDiagnosticMessage>, val errorsCompType: ComparisonType
    )
    ...
    private fun evaluateInRepl(
    ...
    ): List<ResultWithDiagnostics<Pair<List<ReplCompletionVariant>, List<ReplDiagnosticMessage>>>> {
        val compiler = KJvmReplCompilerImpl()
        return snippets.map { (cursor, snippetText, doComplile, doComplete, doErrorCheck) ->
            ...
            val completionRes = if (doComplete && cursor != null) {
                val res = compiler.complete(codeLine, cursor, compilationConfiguration)
            ...
            val errorsRes = if (doErrorCheck) {
                ...
                compiler.analyze(codeLineForErrorCheck, 0, compilationConfiguration).toList()
            } else {
                emptyList()
            }
        ...
        evaluateInRepl(compilationConfiguration, snippets).forEachIndexed { index, res ->
            when (res) {
                ...
                is ResultWithDiagnostics.Success -> {
                    ...
                    checkLists(index, "errors", expectedErrors, errorsRes, errorsCompType)
                }
            }
        }
    }
